...
В директории таблицы увидим data parts, НЕ партиции!
- part_1/
- part_2/
- part_3/
- detached/
...
**Part и Partition**
В ClickHouse есть два понятия, которые часто путают: Part и Partition. Несмотря на схожесть названий, они представляют из себя разные вещи и важно понимать различия. 

1. **Part** — это физический файл на диске, в котором хранится часть данных таблицы, при максимальном упрощении один Part — одна директория со столбцами.\
По-умолчанию:
...
|minmax_*.idx			| Data skipping index: WHERE date BETWEEN ... ClickHouse смотрит min/max в part, позволяет не читать parts вообще			|
